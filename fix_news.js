...
const regex = /<div className="w-full md:w-1\/3 h-64 md:h-auto overflow-hidden">\s*<img\s*src=\{news\.imageUrl[^>]+>\s*<\/div>/g;

const replacement = `<div className="w-full md:w-1/3 h-64 md:h-auto overflow-hidden bg-slate-100 flex-shrink-0">
                           {news.imageUrl && news.imageUrl.match(/(?:youtu\\.be\\/|youtube\\.com\\/(?:embed\\/|v\\/|watch\\?v=|watch\\?.+&v=))([^&?]+)/) ? (
                              <iframe 
                                 className="w-full h-full min-h-[250px] md:min-h-full"
...
fs.writeFileSync('src/pages/news.tsx', content.replace(regex, replacement));
